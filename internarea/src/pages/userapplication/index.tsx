...
const index = () => {
  ...
  const user = useSelector(selectuser); // Ensure user is logged in and populated
  ...
  useEffect(() => {
    const fetchdata = async () => {
      try {
        ...
        if (error.response) {
          setError(`Server error: ${error.response.status} - ${error.response.data?.message || 'Unauthorized or invalid request'}`);
        } else if (error.request) {
          setError("Network error: Unable to reach server. Check your connection or try again later.");
        } else {
  ...
  // Improved filtering: Use user._id if available, else name
  const userApplications = data.filter((app: any) => {
